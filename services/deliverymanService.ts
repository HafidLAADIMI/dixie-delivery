...
export const getDeliverymanByEmail = async (email: string) => {
  try {
    const db = getFirestore();
    const deliverymenRef = collection(db, "deliverymen_applications");
...
export const getDeliverymanDetails = async (deliverymanId) => {
  try {
    ...
    const db = getFirestore();
    const deliverymanRef = doc(db, "deliverymen_applications", deliverymanId);
    ...
    console.log(
      `Successfully retrieved deliveryman data for ID: ${deliverymanId}`,
      data
    );
