...
export default function DeliveriesScreen() {
  ...
  const openDetails = (order) => {
    if (!order || !order.id) {
      console.error(
        "[DeliveriesScreen] Cannot open details - missing order or order ID"
      );
      return;
    }

    console.log(
      `[DeliveriesScreen] Opening order details - userId: "${order.userId}", orderId: "${order.id}"`
    );

    // Check if we have a valid userId that's different from orderId
    if (
      order.userId &&
      ...
      console.log(
        `[DeliveriesScreen] Navigating with userId_orderId format: /delivery/${order.userId}_${order.id}`
      );
      router.push(`/delivery/${order.userId}_${order.id}`);
    } else {
      // If userId is missing, same as orderId, or empty, just use the orderId
      // Your DeliveryDetailsScreen will handle finding the correct path
      console.log(
        `[DeliveriesScreen] No valid userId, navigating with orderId only: /delivery/${order.id}`
      );
        ...
        >
          <TouchableOpacity
            className={`py-3 px-4 relative ${
              activeTab === 0 ? "border-b-2 border-orange-500" : ""
            }`}
          ...
          <TouchableOpacity
            className={`py-3 px-4 relative ${
              activeTab === 1 ? "border-b-2 border-orange-500" : ""
            }`}
          ...
          <TouchableOpacity
            className={`py-3 px-4 relative ${
              activeTab === 2 ? "border-b-2 border-orange-500" : ""
            }`}
          ...
          <TouchableOpacity
            className={`py-3 px-4 relative ${
              activeTab === 3 ? "border-b-2 border-orange-500" : ""
            }`}
